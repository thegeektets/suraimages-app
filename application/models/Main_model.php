...
class Main_model extends CI_Model {
        ...
        public function optimize_all_uploads($search_term,
            ...
            $query = $this->db->query("SELECT DISTINCT * FROM contributor_image_uploads,user_details WHERE user_details.user_id = 
                contributor_image_uploads.user_id AND file_status != 0
                AND file_name LIKE '%".$search_term."%' OR file_keywords LIKE '%".$search_term."%' AND file_type LIKE '%".$image_type."%' AND file_orentiation LIKE '%".$orientation."%'  AND file_license LIKE '%".$license_type."%' AND file_people LIKE '%".$people."%' AND file_category LIKE '%".$category."%' GROUP BY upload_id");
        ...
        public function get_same_shoots($same_shoot){
            $query = $this->db->query("SELECT * FROM contributor_image_uploads,user_details WHERE user_details.user_id = 
                contributor_image_uploads.user_id AND file_status != 0 AND file_same_shoot_code = ".$same_shoot."");
            $image = $query->result_array();
            return array_reverse($image);
        }
        public function get_single_upload($upload_id){
            $query = $this->db->query("SELECT * FROM contributor_image_uploads,user_details WHERE user_details.user_id = 
                contributor_image_uploads.user_id AND file_status != 0 AND upload_id = ".$upload_id."");
            $image = $query->result_array();
            return array_reverse($image);
        }
